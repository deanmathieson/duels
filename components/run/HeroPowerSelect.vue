<template>
  <div class="hp-select w-full h-full flex flex-col items-center justify-center px-6 py-8 overflow-auto">
    ...
    <!-- Floating mid-screen confirm — appears once a power is selected, so it is
         always on-screen on mobile (no scrolling to a clipped footer). The
         wrapper ignores pointer events; only the panel is interactive. -->
    <Transition name="confirm-pop">
      <div v-if="selectedId" class="confirm-float">
        <div class="confirm-card">
          <p class="confirm-label font-engrave">Power selected</p>
          <p class="confirm-name font-engrave">{{ selectedName }}</p>
          <BaseButton variant="gold" size="lg" @click="choose">Confirm Power</BaseButton>
...
/** Display name of the selected power (for the floating confirm panel). */
const selectedName = computed(
  () => powers.value.find((p) => p.id === selectedId.value)?.name ?? 'Power'
)
...
/* Floating mid-screen confirm (mirrors the hero-select pattern). The wrapper
   spans the viewport center but lets clicks fall through; only the panel is
   interactive. */
.confirm-float {
  position: fixed;
  top: 50%;
